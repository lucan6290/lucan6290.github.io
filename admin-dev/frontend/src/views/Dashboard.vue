...
type DashboardView = 'recent' | 'published' | 'draft' | 'category' | 'month'
...
const publishedPosts = computed(() =>
  sortedPosts.value.filter((post) => post.status === 'published')
)
const draftPosts = computed(
  () =>
    sortedPosts.value.filter((post) => post.status === 'draft' || post.status === 'wip')
)
...
function getCategoryLabel(post: PostInfo): string {
  const rawCategories = (post as PostInfo & { categories?: string[] }).categories
  if (rawCategories && rawCategories.length > 0) {
    return rawCategories.filter(Boolean).join(' / ')
  }

  const primary = categoryNameMap[post.category] || post.category || '未分类'
  return post.subCategory ? `${primary} / ${post.subCategory}` : primary
}
...
const listHint = computed(() => {
  const hintMap: Record<DashboardView, string> = {
    ...
    published: `共 ${publishedCount.value} 篇已发布文章`,
    draft: `共 ${draftCount.value} 篇草稿或 WIP 文章`,
...
const columns: DataTableColumns<PostInfo> = [
  ...
  },
  {
    ...
    render(row) {
      ...
      const status = statusMap[row.status] || { text: row.status || '未知', type: 'default' as const }
    ...
    <n-grid :x-gap="16" :y-gap="16" :cols="4" responsive="screen" item-responsive>
      <n-gi span="4 m:2 l:1">
        <n-card
          ...
          @keydown.enter="selectView('published')"
          @keydown.space.prevent="selectView('published')"
        >
          <span class="stat-mark">发布</span>
      ...
      <n-gi span="4 m:2 l:1">
        <n-card
          ...
          :class="{ active: activeView === 'draft' }"
          role="button"
          tabindex="0"
          @click="selectView('draft')"
          @keydown.enter="selectView('draft')"
          @keydown.space.prevent="selectView('draft')"
        >
          <span class="stat-mark">草稿</span>
