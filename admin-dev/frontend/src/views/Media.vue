...
function getImagePreviewUrl(image: ImageInfo): string {
  ...
  return `${API_BASE_URL.replace(/\/$/, '')}/api/admin/v1/assets/file/${encodePath(image.path)}`
}
...
async function handleUpload({ file, onFinish, onError }: any) {
  ...
  try {
    ...
    if (cleanPath.startsWith('blog-content/source/_posts/')) {
      cleanPath = cleanPath.replace('blog-content/source/_posts/', '')
    }
