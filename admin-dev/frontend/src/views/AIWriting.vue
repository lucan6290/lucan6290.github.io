...
const modeOptions: Array<{ value: AIWritingMode; label: string; icon: string; placeholder: string }> = [
  ...
  { value: 'llm', label: 'LLM模式', icon: '◇', placeholder: '描述编辑意图；打开文章后可在编辑器侧边栏执行结构化修改' },
...
const currentInput = computed({
  get: () => {
    ...
    if (aiWriting.activeMode === 'llm') return aiWriting.llmInput
    return aiWriting.agentInput
  },
  set: (value: string) => {
    if (aiWriting.activeMode === 'knowledge') {
      knowledge.question = value
    } else if (aiWriting.activeMode === 'llm') {
      aiWriting.llmInput = value
    } else {
...
const canSubmit = computed(() => {
  ...
  if (aiWriting.activeMode === 'llm') return Boolean(aiWriting.llmInput.trim())
...
async function submitCurrentInput(): Promise<void> {
  ...
  if (aiWriting.activeMode === 'knowledge') {
    ...
    message.info('请在文章编辑器右侧 AI 侧边栏执行结构化编辑')
  } else {
