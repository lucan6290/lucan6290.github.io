...
class TagService:
    ...
    def create_tag(self, payload: TagCreateDTO) -> TagDTO:
        ...
        slug = (payload.slug.strip() if payload.slug else self.slug_service.slug_from_label(label))
        if not self.slug_service.is_valid_slug(slug):
    ...
    def ensure_tags(self, labels: list[str]) -> list[TagDTO]:
        ...
        for label in normalized_labels:
            slug = self.slug_service.slug_from_label(label)
    ...
    def _slug_from_label(self, label: str) -> str:
        return self.slug_service.slug_from_label(label)
